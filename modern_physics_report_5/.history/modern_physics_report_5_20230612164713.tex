...
\section{Theory}
	...
	XRD 분석이란 X-ray를 원하는 시편에 회절 시켜 시편의 내부 정보를 그래프로 나타내는 분석법이다. 보통 최종 결과물은 아래의 그림 그래프와 같다. X축은 로 Y축은 intensity라는 값으로 그래프로 나타내며 여러 상들의 고유한 각도에서 peak값이 나타나 시편 속에 어떠한 phase로 이루어져 있는지 확인할 수 있는 분석 방법입니다.
