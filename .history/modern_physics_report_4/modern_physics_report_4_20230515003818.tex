...
\section{실험 방법}
	...
	K의 옆에 있는 필라멘트를 가열하면 K에서 방출 된 열전자는 G$_2$에 의해 가속되어 운동에너지가 증가하게 된다. 이 음전하를 띈 열전자가 G$_2$를 통과한 후 양극인 A에 걸린 역전압 $V_0$에 의해 약간 감속이 되며 A에 흡수된다. (G$_2$에 걸리는 가속전압을 증가시키면 더 많은 열전자가 에 도달하여 플레이트 전류는 증가한다)
