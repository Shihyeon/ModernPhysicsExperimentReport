...
\section{이론}
  ...
  하고, 가속 전압이 증가하면 전류는 다시 증가하여 전압이 제2차 한계 값에 이르면 다시 급속히 감소하 Top : Simplified term diagram for neon. 고, 이후에는 이와 같은 전류의 극대 값이 반복된다. Bottom : the electron current flowing to 이와 같이, 네온 원자에 속박된 전자의 에너지 준위가
